...
xlabel('S')
ylabel('PDF(S)')
%title(['$\tau = $' num2str(1.242)], 'Interpreter', 'LaTeX')
XLm = [1, 10^ceil(log10(L))];log10(max(pdf))/sum(pdf)
YLm = [10^floor(log10(pdf(end)/sum(pdf))), 1]%10^ceil(log10(max(pdf))/sum(pdf))]
% axis(XLm, YLm)
set(gca,'xtick',XLm, 'ytick', YLm, 'xlim', XLm, 'Ylim', YLm,'fontsize',20)
box off
if flag == 1
    [burstMax, burstMin] = EXCLUDE(burst,'setmin', 100);
    ...
    title(['$\tau$=', num2str(alpha,3)], 'Interpreter' ,'LaTeX', 'FontSize', 24)
elseif flag == 5
...
% axis(XLm, YLm)
set(gca,'xtick',XLmt, 'ytick', YLmt, 'xlim', XLmt, 'Ylim', YLmt, 'fontsize',24)
if flag == 1
    flag
    ...
    title(['$\alpha$=',num2str(beta,3)], 'Interpreter', 'LaTeX', 'FontSize',24)
elseif flag == 5
    flag
...
if flag == 1 || flag ==5
    ...
    xlabel('D (s)')
    ylabel('<S>')
    set(gca,'xtick',XLmt, 'ytick', XLm, 'Xlim', XLmt, 'Ylim', XLm,'fontsize',20)
    title(['$\beta_o$ = ',num2str(fit_sigma(1),3), ' $\beta_p=$ ', num2str(sigma,3)], 'Interpreter','LaTeX','FontSize',24);
end
% figure
% for i = 5:50%max(TT)
